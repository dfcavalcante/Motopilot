...
const ReportCard = ({ report, onOpen, onDownload, onDelete, onConclude }) => (
  <Card
    sx={{
      height: '100%',
      display: 'flex',
  ...
  >
    ...
    <Box sx={{ p: 2, display: 'flex', gap: 1, backgroundColor: '#fafafa' }}>
      ...
      {/* Botão de concluir só aparece se o relatório ainda não estiver concluído, só o gerente pode fazer isso */}
      <Button
        size="small"
        variant="outlined"
        color="success"
        onClick={() => onConclude(report.id)}
        sx={{ borderColor: '#e8f5e9', color: '#2e7d32', borderRadius: '8px', minWidth: '45px' }}
      >
